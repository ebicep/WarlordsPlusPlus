...
class DamageDoneHudElement : AbstractHudElement(Renderer(), 0, 0) {
    ...
    class Renderer : RenderHelperGui() {
        ...
        override fun render0() {
            val damage = "Damage: ${WarlordsPlayer.damageDoneCounter}"
            createPose {
                translateX(-2)
            ...
            createPose {
                translateY(-2)
                guiGraphics?.drawString(Minecraft.getInstance().font, damage, 0, 0, Colors.RED.convertToArgb(), false)
            }
        }
    }

    override fun width(): Int {
        return Minecraft.getInstance().font.width("Damage: ")
    }
